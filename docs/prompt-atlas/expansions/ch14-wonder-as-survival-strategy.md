...
> **Canonical:** [PROMPT_ATLAS.md#ch14-wonder-as-survival-strategy](../../../PROMPT_ATLAS.md#ch14-wonder-as-survival-strategy) · **Prompts:** [`prompts/ch14.yaml`](../prompts/ch14.yaml) · **Part:** [VII](part-vii.md)
...
"Compose a 5-minute experience in which a user perceives {{ecosystem}}
 from the perspective of a {{bee|whale|mushroom}}. Include the data
...
"Generate a one-page meditation that places a single human life in the
 context of {{deep_time}}. Avoid grandiosity; end with a single,
 actionable, ordinary kindness."
```

## Anti-patterns

- **Engagement-tuned awe.** Wonder optimized for clicks becomes addiction.
- **Spectacle without quiet.** Awe collapses without the silence to receive it.
- **Awe as bypass.** Cosmic perspective used to dismiss present injustice.
- **Single-steward curation.** A cathedral run by one taste calcifies.
...
- [`losses_geom.py`](../../../src/losses_geom.py) — Wasserstein and MMD distances let you visualize *distributions* (which is closer to awe-shaped data) instead of single numbers.
- [`MACP bus`](../../../src/macp/bus.py) — multi-steward agent topology for cathedral curation.
- [`CoLearningMemoryStore`](../../../src/vectorstore/qdrant_store.py) — long-running, slowly-evolving archive of installations.

## Guide for AI & Humanity

- **Awe needs silence.** Build the quiet first.
- **Refuse engagement metrics for wonder.** Use recurrence-over-years instead.
- **Awe and justice braid together.** Perspective without action is escapism.
- **Local before planetary.** Cathedrals belong first to the neighborhood that walks past them.
